          ...
          <div className="hdr">
            ...
            <div style={{flex:1,margin:"0 12px",position:"relative"}}>
              ...
              {showSearch && searchQuery && (searchResults.users.length > 0 || searchResults.posts.length > 0) && (
                <div style={isDesktop ? {position:"absolute",top:"100%",left:0,right:0,background:"#121820",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"12px",marginTop:"4px",zIndex:1000,maxHeight:"300px",overflowY:"auto"} : {position:"fixed",top:"120px",left:"16px",right:"16px",background:"#121820",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"12px",zIndex:1000,maxHeight:"300px",overflowY:"auto"}}>
                  {searchResults.users.length > 0 && <div style={{padding:"8px 12px 4px",fontSize:"10px",color:"#556677",textTransform:"uppercase",letterSpacing:"1px"}}>{t.users}</div>}
                  {searchResults.users.map(u => (
                    <div key={u.id} onClick={() => { supabase.from('profiles').select('*').eq('id', u.id).single().then(({data}) => { if (data) setViewProfile({id:data.id,name:data.full_name,avatar:data.full_name?.substring(0,2).toUpperCase(),avatar_url:data.avatar_url,position:data.position||'',country:data.country||'',city:data.city||'',age:data.age||'',bio:data.bio||'',verified:data.verified||false,followers:0,following:0,height:data.height,weight:data.weight,dominant_foot:data.dominant_foot,goal:data.goal,account_type:data.account_type}); }); setShowSearch(false); setSearchQuery(''); setTab('home'); }} style={{display:"flex",alignItems:"center",gap:"10px",padding:"8px 12px",cursor:"pointer"}}>
                ...
                {(Array.isArray(debate.options) ? debate.options : JSON.parse(debate.options)).map((opt, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => { if (!closed) voteDebate(i); }} style={{position:'relative',borderRadius:'12px',overflow:'hidden',cursor:(userVote === null && !closed) ? 'pointer' : 'default'}}>
                      {showResults && <div style={{position:'absolute',inset:0,background:voted?'rgba(0,230,118,0.15)':'rgba(255,255,255,0.03)',width:`${pct}%`}}/>}
                      <div style={{position:'relative',padding:'13px 16px',display:'flex',alignItems:'center',justifyContent:'space-between',border:voted?'1.5px solid #00E676':'1px solid rgba(255,255,255,0.12)',borderRadius:'12px',transition:'all 0.2s'}}>
                        <span style={{fontSize:'14px',fontWeight:'700',color:voted?'#00E676':'#ECEFF4',display:'inline-flex',alignItems:'center',gap:'4px'}}>{(userVote===null && !closed)?<CircleCheck size={14} />:''}{opt}</span>
                        {showResults && <span style={{fontSize:'14px',fontWeight:'800',color:voted?'#00E676':'#8899A6'}}>{pct}%</span>}
                      </div>
                    </div>
                  );
                })}
            ...
            <div className="profile">
              ...
              {myPosts.length > 0 && (
                <div style={{width:'100%',marginTop:'20px',textAlign:'left'}}>
                  ...
                  {myPosts.map(p => (
                    <div key={p.id} style={{background:'#0a0e14',border:'1px solid rgba(255,255,255,0.06)',borderRadius:'14px',padding:'14px',marginBottom:'10px',cursor:'pointer',maxWidth:'600px'}} onClick={() => setViewPost({
                      id: 'real-' + p.id,
                      userId: p.user_id,
                      name: userProfile?.full_name || 'Usuario',
                      av: userProfile?.full_name ? userProfile.full_name.substring(0,2).toUpperCase() : 'TU',
